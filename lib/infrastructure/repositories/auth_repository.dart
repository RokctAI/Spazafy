...
class AuthRepository implements AuthFacade {
    ...
    try {
      ...
      try {
        ...
        final response = await client.post(
          ...
          data: {'email': email, 'name': displayName, 'id': id, 'avatar': avatar},
        );
        return ApiResult.success(data: LoginResponse.fromJson(response.data));
      } catch (e2) {
  ...
  Future<ApiResult<VerifyPhoneResponse>> verifyPhone({
    ...
    try {
      ...
      return ApiResult.success(data: VerifyPhoneResponse.fromJson(response.data));
    } catch (e) {
      // Fallback to V1
      try {
        final client = dioHttp.client(requireAuth: false);
  ...
  Future<ApiResult<RegisterResponse>> forgotPassword({required String email}) async {
    try {
      ...
      try {
        final client = dioHttp.client(requireAuth: false);
        final response = await client.post(
          AppValidators.isValidEmail(email) ? '/api/v1/auth/forgot/email-password' : '/api/v1/auth/forgot/password',
  ...
  Future<ApiResult<VerifyData>> forgotPasswordConfirm({
    ...
    try {
      ...
      try {
        final client = dioHttp.client(requireAuth: false);
        final response = await client.post('/api/v1/auth/forgot/email-password/$verifyCode?email=$email');
        return ApiResult.success(data: VerifyData.fromJson(response.data["data"]));
      } catch (e2) {
        return ApiResult.failure(error: AppHelpers.errorHandler(e2));
      }
    }
  ...
  Future<ApiResult<VerifyData>> sigUpWithData({required UserModel user}) async {
    try {
      ...
      return ApiResult.success(data: VerifyData.fromJson(res.data['data'] ?? res.data));
    } catch (e) {
      // Fallback to V1
      try {
        final client = dioHttp.client(requireAuth: false);
        var res = await client.post('/api/v1/auth/after-verify', data: user.toJsonForSignUp());
  ...
  Future<ApiResult<VerifyData>> sigUpWithPhone({required UserModel user}) async {
    try {
      ...
      try {
        final client = dioHttp.client(requireAuth: false);
        var res = await client.post('/api/v1/auth/verify/phone', data: user.toJsonForSignUp(typeFirebase: true));
        return ApiResult.success(data: VerifyData.fromJson(res.data["data"]));
      } catch (e2) {
        return ApiResult.failure(error: AppHelpers.errorHandler(e2));
      }
  ...
  Future<ApiResult> sigUp({required String email}) async {
    try {
      ...
      try {
        final client = dioHttp.client(requireAuth: false);
        await client.post('/api/v1/auth/register', queryParameters: {'email': email});
        return const ApiResult.success(data: null);
      } catch (e2) {
        return ApiResult.failure(error: AppHelpers.errorHandler(e2));
      }
  ...
  @override
  Future<ApiResult<CheckPhoneResponse>> checkPhone({required String phone}) async {
    final data = {'phone': phone.replaceAll("+", "")};
    try {
      final client = dioHttp.client(requireAuth: false);
      final response = await client.post('/api/v1/auth/check/phone', queryParameters: data);
      return ApiResult.success(data: CheckPhoneResponse.fromJson(response.data));
    } catch (e) {
      return ApiResult.failure(error: AppHelpers.errorHandler(e));
    }
  }
