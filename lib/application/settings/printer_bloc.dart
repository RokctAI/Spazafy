...
class PrinterBloc extends Bloc<PrinterEvent, PrinterState> {
  ...
  Future<void> _onScan(
    ...
    try {
      final devices = await repository.scanDevices();
      emit(state.copyWith(status: PrinterStatus.scanSuccess, devices: devices));
    } catch (e) {
      emit(
        state.copyWith(
          status: PrinterStatus.scanFailure,
  ...
  Future<void> _onDisconnect(
    ...
    emit(
      PrinterState(status: PrinterStatus.disconnected, devices: state.devices),
    );
  }

  Future<void> _onTestPrint(
    TestPrintEvent event,
    Emitter<PrinterState> emit,
  ) async {
